...
alter table wallet_balance_observations
  ...
  add constraint wallet_balance_observations_decimals_check
...
begin
  ...
  insert into provider_health (
    ...
    coalesce(nullif(p_health->>'provider', ''), 'ethereum-rpc'),
    v_checked_at,
  ...
  loop
    begin
      v_wallet_id := null;
      v_asset_id := null;
      select tw.id, a.id, a.decimals, wam.balance_query_type, wam.token_identifier
      into v_wallet_id, v_asset_id, v_asset_decimals, v_mapping_type, v_token_identifier
      ...
      where tw.id::text = v_item->>'trackedWalletId'
        ...
        and tw.chain_code = 'ethereum'
        and a.chain_code = 'ethereum'
      ...
      if v_wallet_id is null
        or v_item->>'provider' <> 'ethereum-rpc'
        ...
        or coalesce(v_item->>'blockHash', '') !~ '^0x[0-9a-fA-F]{64}$'
      ...
      if v_decimals not between 0 and 18
        ...
        or (v_mapping_type = 'native' and v_decimals <> 18)
        or (v_mapping_type = 'erc20' and (
          v_token_identifier is null
          or lower(v_token_identifier) <> lower(coalesce((select contract_address from assets where id = v_asset_id), ''))
        ))
      ...
      insert into wallet_balance_observations (
        ...
        v_wallet_id, v_asset_id, 'ethereum-rpc', v_block_number,
